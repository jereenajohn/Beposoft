...
class cso_today_OrderList extends StatefulWidget {
  var status;
  cso_today_OrderList({super.key, required this.status});

  @override
  State<cso_today_OrderList> createState() => _cso_today_OrderListState();
}

class _cso_today_OrderListState extends State<cso_today_OrderList> {
...
Future<void> fetchOrderData() async {
  ...
  if (orderData['status'] != "Order Request by Warehouse") {
    // ❌ Exclude if family is 'bepocart'
    String family = orderData['family']?.toString().toLowerCase() ?? '';
    if (family != 'bepocart') {
      newOrders.add({
        'id': orderData['id'],
