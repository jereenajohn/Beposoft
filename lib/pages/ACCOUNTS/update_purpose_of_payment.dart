...
class update_purpose_of_payment extends StatefulWidget {
  final id;
  const update_purpose_of_payment({super.key, required this.id});

  @override
  State<update_purpose_of_payment> createState() => _update_purpose_of_paymentState();
}

class _update_purpose_of_paymentState extends State<update_purpose_of_payment> {
  @override
  void initState() {
    super.initState();
    getpurpose();
  ...
  void updatepurpose() async {
    ...
    try {
      var response = await http.put(
        Uri.parse('$api/apis/purpose/update/${widget.id}/'),
        headers: {
          'Authorization': 'Bearer $token',
        },
        body: {
     ...
     ;
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => update_purpose_of_payment(id: widget.id)));
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
