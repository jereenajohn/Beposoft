...
class FamilywisedateSalesreport extends StatefulWidget {
  var family;
  var startdate;
  var enddate;
  FamilywisedateSalesreport({super.key, required this.family, this.startdate, this.enddate});

  @override
  State<FamilywisedateSalesreport> createState() => _FamilywisedateSalesreportState();
}

class _FamilywisedateSalesreportState extends State<FamilywisedateSalesreport> {
...
Future<void> fetchOrderData() async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      DateTime? startDate = widget.startdate != null
          ? DateTime.parse(widget.startdate)
          : null;
      DateTime? endDate = widget.enddate != null
      ...
      for (var orderData in ordersData) {
        ...
        try {
          parsedOrderDate = DateFormat('yyyy-MM-dd').parse(rawOrderDate);
          formattedOrderDate = DateFormat('yyyy-MM-dd').format(parsedOrderDate);
        } catch (e) {}

        bool isInDateRange = true;
        if (startDate != null && endDate != null && parsedOrderDate != null) {
          isInDateRange = (parsedOrderDate.isAtSameMomentAs(startDate) ||
              parsedOrderDate.isAtSameMomentAs(endDate) ||
              (parsedOrderDate.isAfter(startDate) &&
                  parsedOrderDate.isBefore(endDate)));
        }

        if (orderData['family'] == widget.family &&
