...
class familysummary extends StatefulWidget {
  var status;
  var family;
  familysummary({super.key, required this.status, required this.family});

  @override
  State<familysummary> createState() => _familysummaryState();
}

class _familysummaryState extends State<familysummary> {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      for (var orderData in ordersData) {
        ...
        // Filter for today + status/family if needed
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
       onWillPop: () async {
        Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => ceo_family_summary()),
    );
