...
import 'package:excel/excel.dart' hide Border; // 👈 hide the conflicting Border
import 'package:flutter/material.dart';
...
class purchase_List extends StatefulWidget {
  var status;
  purchase_List({super.key, required this.status});

  @override
  State<purchase_List> createState() => _purchase_ListState();
}

class _purchase_ListState extends State<purchase_List> {
...
Future<void> fetchOrderData() async {
  try {
    ...
    String url = '$api/api/warehouse/order/view/';
    ...
    if (response.statusCode == 200) {
      ...
      final List ordersData = responseData['data'];
...
void _filterAndSortOrdersByWarehouse(int? warehouseId) {
  setState(() {
    if (warehouseId == null) {
      filteredOrders = orders;
    } else {
      filteredOrders = orders
          .where((order) => order['warehouses_name'] == Warehouses.firstWhere((w) => w['id'] == warehouseId)['name'])
          .toList();
    }
    // Sort by order_date descending
    filteredOrders.sort((a, b) {
      DateTime dateA = DateTime.tryParse(a['order_date'] ?? '') ?? DateTime(2000);
      DateTime dateB = DateTime.tryParse(b['order_date'] ?? '') ?? DateTime(2000);
  ...
  void _filterOrders(String query) {
    ...
    setState(() {
      ...
      if (query.isEmpty) {
        ...
        filteredOrders = orders.where((order) {
          final customerName =
              order['warehouses_name']?.toString().toLowerCase() ?? '';
          final invoice = order['invoice']?.toString().toLowerCase() ?? '';
          final manageStaff =
              order['receiiver_warehouse_name']?.toString().toLowerCase() ?? '';
  ...
  Widget build(BuildContext context) {
   ...
   Navigator.pushReplacement(
        ...
        body: Column(
          children: [
             Padding(
 padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 8),  child: SizedBox(
  ...
  setState(() {
    ...
    selectedwarehouseName = Warehouses.firstWhere((element) => element['id'] == newValue)['name'];
  });
  _filterAndSortOrdersByWarehouse(newValue);
},
    ...
    Padding(
      ...
      child: DropdownButtonFormField<String>(
        ...
        decoration: InputDecoration(
          ...
          filled: true,
          fillColor: Colors.blue.shade50, // subtle background
                                    ...
                                    Padding(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          SizedBox(height: 4.0),
                                          Text(
                                            'Warehouse: ${order['warehouses_name'] ?? ''}',
                                            style: TextStyle(fontSize: 13),
                                          ),
                                          SizedBox(height: 4.0),
                                          Text(
                                            'Receiver Warehouse: ${order['receiiver_warehouse_name'] ?? ''}',
                                            style: TextStyle(fontSize: 13),
                                          ),
                                          SizedBox(height: 4.0),
                                          Row(
