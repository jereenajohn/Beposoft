...
class _recipt_ReportState extends State<recipt_Report> {
  ...
  try {
    ...
    var response = await http.get(
      Uri.parse('$api/api/allreceipts/view/'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
      },
    ...
    if (response.statusCode == 200) {
      ...
      final receipts = parsed['receipts'] as List<dynamic>;

      for (var reportData in receipts) {
        String receiptType = reportData['receipt_type'] ?? 'unknown';
        ...
        reciptList.add({
          ...
          'bank_name': reportData['bank_name'] ?? '',
          'order_name': reportData['order_name'] ?? '',
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                 ...
                                 if (reportData['order_name'] != null &&
    reportData['order_name'].toString().isNotEmpty)
  _buildRow('Invoice:', reportData['order_name']),
                                    // _buildRow(
