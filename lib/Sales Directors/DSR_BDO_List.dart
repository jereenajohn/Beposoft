...
class DsrBdoList extends StatefulWidget {
  const DsrBdoList({super.key});

  @override
  State<DsrBdoList> createState() => _DsrBdoListState();
}

class _DsrBdoListState extends State<DsrBdoList> {
  ...
  void _applyFilters() {
    ...
    final selectedState = selectedStateName.trim().toLowerCase();
    ...
    filteredDsrList = dsrList.where((item) {
      ...
      final stateName = item["state_name"]?.toString().toLowerCase() ?? "";
      final districtName =
          item["district_name"]?.toString().toLowerCase() ?? "";
      final createdByName =
          item["created_by_name"]?.toString().toLowerCase() ?? "";
      final note = item["note"]?.toString().toLowerCase() ?? "";
      final callStatus =
          item["call_status"]?.toString().trim().toLowerCase() ?? "";
      final dsrStatus = item["status"]?.toString().trim().toLowerCase() ?? "";
      ...
      final matchesState =
          selectedState.isEmpty || stateName.contains(selectedState);
      final matchesCustomer =
          customer.isEmpty || customerName.contains(customer);

      final matchesStaff = selectedStaffName.trim().isEmpty
          ? true
          : createdByName == selectedStaffName.trim().toLowerCase();

      bool matchesSummary = true;

      if (selectedSummaryFilter == "active") {
        matchesSummary = callStatus == "active";
      } else if (selectedSummaryFilter == "productive") {
        matchesSummary = callStatus == "productive";
      } else if (selectedSummaryFilter == "created") {
        matchesSummary = dsrStatus == "dsr created";
      } else if (selectedSummaryFilter == "approved") {
        matchesSummary = dsrStatus == "dsr approved";
      } else if (selectedSummaryFilter == "confirmed") {
        matchesSummary = dsrStatus == "dsr confirmed";
      } else if (selectedSummaryFilter == "rejected") {
        matchesSummary = dsrStatus == "dsr rejected";
      }
    ...
    for (final item in filteredDsrList) {
      ...
      final invoiceAmount =
          double.tryParse((item["invoice_amount"] ?? 0).toString()) ?? 0.0;
      ...
      if (callStatus == "productive") {
        filteredProductiveCount++;
      }

      if (dsrStatus == "dsr approved") {
        filteredDsrApprovedCount++;
      } else if (dsrStatus == "dsr confirmed") {
        filteredDsrConfirmedCount++;
      } else if (dsrStatus == "dsr rejected") {
        filteredDsrRejectedCount++;
      } else if (dsrStatus == "dsr created") {
        filteredDsrCreatedCount++;
      }
    }
  ...
  Future<void> fetchDsrList({bool isRefresh = false}) async {
    try {
      ...
      final uri = Uri.parse('$api/api/sales/analysis/family/$familyId/')
        ...
        if (parsed is Map) {
          ...
          if (parsed["results"] is Map) {
            ...
            tempCallDurationAvg8hrs = double.tryParse(
                    (resultMap["average_call_duration"] ?? 0).toString()) ??
                0.0;
  ...
  Future<void> exportToExcel() async {
    try {
      ...
      final summaryData = [
        ...
        [
          "AVG DURATION (8 HRS)",
          "${filteredCallDurationAvg8hrs.toStringAsFixed(2)} min"
        ],
        [
          "DURATION % (8 HRS)",
  ...
  Widget buildProductDetails(int itemId, List<Map<String, dynamic>> products) {
    ...
    final List<Map<String, dynamic>> visibleProducts =
        isExpanded ? products : [products.first];
