...
Future<void> getexpenselist() async {
  try {
    final token = await gettokenFromPrefs();
    final response = await http.get(
      Uri.parse('$api/api/get/expense/${widget.id}/'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
      },
    ...
    if (response.statusCode == 200) {
      ...
      final Map<String, dynamic>? data =
          (parsed is Map && parsed['data'] is Map) ? parsed['data'] as Map<String, dynamic> : null;
      ...
      // Parse date (keep null-safe)
      DateTime? expenseDate;
      final String? dateStr = data['expense_date']?.toString();
      if (dateStr != null && dateStr.isNotEmpty) {
        expenseDate = DateTime.tryParse(dateStr);
      }

      if (!mounted) return;
      setState(() {
        // IDs now come directly as ints
        selectedCompanyId = data['company'] as int?;
        selectedstaffId  = data['payed_by'] as int?;
        selectedbankId   = data['bank'] as int?;
        selectedCategoryId = data['category'] as int?;
        selectedEmiId    = data['loan'] as int?;
        selectedEmiName  = data['emi_name'] as String?;

        // Purpose now only has the ID
        selectedPurposeId   = data['purpose_of_payment'] as int?;
        selectedPurposeName = data['purpose_of_payment_name'] as String?;

        // Asset type & expense type
        selectedtype  = (data['asset_types'] ?? 'expenses').toString();
        selectedtype1 = (data['expense_type'] ?? 'miscellaneous').toString();

        // Text fields
        transactionid.text = (data['transaction_id'] ?? '').toString();
        proname.text       = (data['name'] ?? '').toString();
        quantity.text      = data['quantity'] == null ? '' : data['quantity'].toString();
