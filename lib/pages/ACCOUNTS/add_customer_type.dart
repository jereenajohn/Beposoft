...
import 'package:beposoft/pages/ACCOUNTS/update_customer_type.dart';
...
class add_customer_type extends StatefulWidget {
  const add_customer_type({super.key});

  @override
  State<add_customer_type> createState() => _add_customer_typeState();
}

class _add_customer_typeState extends State<add_customer_type> {
  drower d = drower();
  ...
  Future<void> Addcustomertype(
    BuildContext scaffoldContext,
  ) async {
    final token = await gettoken();
    try {
      final response = await http.post(Uri.parse('$api/api/customer-types/'),
          headers: {
      ...
      if (response.statusCode == 201) {
        ScaffoldMessenger.of(scaffoldContext).showSnackBar(
          SnackBar(
            ...
            content: Text('Customer type added Successfully.'),
          ),
        );
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => add_customer_type()));
      } else {
        ScaffoldMessenger.of(scaffoldContext).showSnackBar(
          SnackBar(
            backgroundColor: Colors.red,
            content: Text('Adding customer type failed.'),
          ),
        );
      }
    } catch (e) {}
  ...
  Future<void> getcustomertype() async {
    final token = await gettoken();
    try {
      final response = await http.get(Uri.parse('$api/api/customer-types/'), headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      });
                            ...
                            child: GestureDetector(
                              onTap: () {
                                Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                        builder: (context) =>
                                            update_customer_type(id: type[i]['id'])));
                              },
                              child: Image.asset(
                                "lib/assets/edit.jpg",
                                width: 20,
