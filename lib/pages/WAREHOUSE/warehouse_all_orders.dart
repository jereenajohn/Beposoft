...
class warehouse_OrderList extends StatefulWidget {
  var status;
  warehouse_OrderList({super.key, required this.status});

  @override
  State<warehouse_OrderList> createState() => _warehouse_OrderListState();
}

class _warehouse_OrderListState extends State<warehouse_OrderList> {
    ...
    if (response.statusCode == 200) {
      ...
      for (var orderData in ordersData) {
        ...
        try {
          ...
          if (orderData['status'] != "Invoice Created" && orderData['status'] != "Invoice Approved" && orderData['status'] != "Waiting For Confirmation" && orderData['status'] != "Invoice Rejected")  {
            newOrders.add({
              'id': orderData['id'],
