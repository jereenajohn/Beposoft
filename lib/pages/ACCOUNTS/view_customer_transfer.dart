...
import 'package:beposoft/pages/ACCOUNTS/update_customer_transfer.dart';
import 'package:beposoft/pages/ACCOUNTS/update_recipt.dart';
import 'package:beposoft/pages/ACCOUNTS/update_refund.dart';
...
class customertransferList extends StatefulWidget {
  const customertransferList({super.key});

  @override
  State<customertransferList> createState() => _customertransferListState();
}

class _customertransferListState extends State<customertransferList> {
...
Future<void> getrefundReport() async {
  setState(() {});
  ...
  try {
    ...
    final response = await http.get(
      Uri.parse('$api/api/advance/transfer/create/'),
      headers: {
        'Authorization': 'Bearer $token',
        'Content-Type': 'application/json',
      },
    ...
    if (response.statusCode == 200) {
      ...
      // ✅ IMPORTANT: Extract list properly
      final List dataList = parsed['data'] ?? [];
      ...
      for (var reportData in dataList) {
       receiptList.add({
  'id': reportData['id'],
  'send_from_name': reportData['send_from_name'] ?? '',
  'send_to_name': reportData['send_to_name'] ?? '',
  'amount': double.tryParse(reportData['amount'].toString()) ?? 0.0,
  'date': reportData['date'] ?? '',
  'remark': reportData['note'] ?? '',
  'images': reportData['amount_transfer_images'] ?? [],
});

      }

      setState(() {
        allSalesReportList = receiptList;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Failed to fetch refund data'),
          duration: Duration(seconds: 2),
        ),
      );
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Error fetching refund data'),
        duration: Duration(seconds: 2),
      ),
    );
  } finally {
...
Future<void> _navigateBack() async {
  ...
  final List images = reportData['images'] ?? [];
  ...
  return GestureDetector(
    onTap: () {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => updatecustomertransfer(
            id: reportData['id'],
          ),
        ),
      );
     ...
     color: const Color(0xFFF5F9FF),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            _buildRow('From:', reportData['send_from_name']),
            _buildRow('To:', reportData['send_to_name']),
            _buildRow('Amount:', reportData['amount']),
            _buildRow('Date:', reportData['date']),
            if ((reportData['remark'] ?? '').toString().isNotEmpty)
              _buildRow('Remark:', reportData['remark']),
    
            // ✅ IMAGE SECTION (MUST be here)
            if (images.isNotEmpty) ...[
              ...
              GridView.builder(
                ...
                gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                  crossAxisCount:
                      MediaQuery.of(context).size.width < 600 ? 3 : 5,
                  crossAxisSpacing: 6,
                  mainAxisSpacing: 6,
                ),
                itemBuilder: (context, index) {
                  final imgUrl = '$api${images[index]['image']}';
