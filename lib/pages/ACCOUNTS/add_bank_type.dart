...
class add_bank_type extends StatefulWidget {
  const add_bank_type({super.key});

  @override
  State<add_bank_type> createState() => _add_bank_typeState();
}

class _add_bank_typeState extends State<add_bank_type> {
  @override
  void initState() {
    super.initState();
    getbanktype();
...
Future<void> getbanktype() async {
  ...
  void addbanktype(BuildContext context) async {
    final token = await gettokenFromPrefs();
    ...
    try {
      ...
      if (response.statusCode == 201) {
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => add_bank_type()));
