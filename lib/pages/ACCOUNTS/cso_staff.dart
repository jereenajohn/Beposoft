...
class cso_staff_list extends StatefulWidget {
  const cso_staff_list({super.key});

  @override
  State<cso_staff_list> createState() => _cso_staff_listState();
}

class _cso_staff_listState extends State<cso_staff_list> {
  drower d = drower();
...
sta.addAll(
  stafflist.where((staff) {
    final familyName =
        staff['family_name']?.toString().toLowerCase().trim() ?? '';
    return familyName != 'bepocart';
  }).toList(),
);            currentPage += 1;
          } else {
sta = stafflist.where((staff) {
  final familyName = staff['family_name']?.toString().toLowerCase().trim() ?? '';
  return familyName != 'bepocart';
}).toList();
            currentPage = 1;
          }
